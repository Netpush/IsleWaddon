...
window.drawThings = function () {
    ...
    Object.keys(window.thingsToDraw).forEach(function (key) {
        ...
        if (thingsToDraw[key].name != "sun_carp" && thingsToDraw[key].name.indexOf("Gilde Gift") == -1) {
            ...
            jQuery(".minimapName." + window.thingsToDraw[key].name).eq(0).css("top", top - 12);
            jQuery(".minimapName." + window.thingsToDraw[key].name).eq(0).css("left", left - (window.thingsToDraw[key].name.length) * 4);
